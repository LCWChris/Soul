...
router.get("/user/:userId", async (req, res) => {
  try {
    ...
    const allWords = await BookWord.find({
    ...
    // 簡化的等級統計
    const levelMap = {};
    const levelWords = await BookWord.find({
      learning_level: { $exists: true, $ne: null, $ne: "" },
    }).lean();
    ...
    const allLevels = Object.entries(levelMap).map(([name, total]) => ({
      _id: name,
      total,
    }));
    ...
    const categoryProgress = allCategories.map((cat) => {
      const userCatStats = userStats.categoryStats[cat._id] || {
        learned: 0,
        mastered: 0,
      };
      return {
        ...
        percentage:
          cat.total > 0
            ? Math.round((userCatStats.learned / cat.total) * 100)
            : 0,
      };
    });
    ...
    console.log(
      "📊 清理後的分類數據:",
      categoryProgress.map((cat) => cat.name)
    );

    // 構建等級統計
    const levelProgress = allLevels.map((level) => {
      const userLevelStats = userStats.levelStats[level._id] || {
        learned: 0,
        mastered: 0,
      };
      const displayNames = {
      ...
      return {
        ...
        percentage:
          level.total > 0
            ? Math.round((userLevelStats.learned / level.total) * 100)
            : 0,
      };
    });
        ...
        if (activityByDate[dateKey]) {
          ...
          if (record.timeSpent) {
            ...
            if (timeInSeconds > 1000) {
              ...
              console.log(
                `⚠️ 自動修正舊時間數據: ${record.timeSpent}ms → ${timeInSeconds}秒`
              );
            }
    ...
    const recentActivity = Object.keys(activityByDate)
      ...
      .filter(
        (activity) => activity.wordsLearned > 0 || activity.timeSpent > 0
      ); // 過濾掉沒有活動的日期
    ...
    const progressPercentage =
      totalWordsCount > 0
        ? Math.round((userStats.totalWordsLearned / totalWordsCount) * 100)
        : 0;
    ...
    if (correctedTotalStudyTime > 10000) {
      ...
      console.log(
        `⚠️ 檢測到異常的總學習時間: ${correctedTotalStudyTime}分鐘,暫時顯示為 0`
      );
...
router.post("/activity", async (req, res) => {
  try {
    const { userId, wordId, action, difficulty, timeSpent, isCorrect } =
      req.body;
...
router.get("/history/:userId", async (req, res) => {
  try {
    ...
    const progress = await LearningProgress.findOne({ userId }).populate(
      "learningRecords.wordId"
    );
...
router.get("/today-tasks/:userId", async (req, res) => {
  try {
    const { userId } = req.params;
    console.log(`📊 請求今日任務狀態, userId: ${userId}`);
    ...
    if (
      !progress ||
      !progress.learningRecords ||
      progress.learningRecords.length === 0
    ) {
      console.log("🤷‍♂️ 找不到用戶進度或學習記錄");
      return res.json({
        completedTasks: 0,
        totalTasks: 3,
        tasks: {
    ...
    // 篩選出今天的學習記錄
    const todayRecords = progress.learningRecords.filter((record) => {
      const recordDate = new Date(record.timestamp);
      return recordDate >= today && recordDate < tomorrow;
    });

    console.log(`📝 今天有 ${todayRecords.length} 筆學習記錄`);

    // 檢查各項任務是否完成
    const tasks = {
      learn: todayRecords.some((r) => r.action === "learn"),
      review: todayRecords.some((r) => r.action === "review"),
      quiz: todayRecords.some(
        (r) => r.action === "quiz" && r.isCorrect === true
      ),
    };
    ...
    console.log("✅ 成功獲取今日任務狀態:", response);
    res.json(response);
  } catch (error) {
    console.error("❌ 獲取今日任務狀態失敗:", error);
    res
      .status(500)
      .json({ error: "獲取今日任務狀態失敗", message: error.message });
  }
});
