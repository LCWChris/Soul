...
function gradeQuiz(quiz, answers) {
  ...
  const gradable = quiz.questions.filter(
    (q) => !["video_response", "hotspot"].includes(q.type)
  );
  ...
  useEffect(() => {
    const fetchQuizData = async () => {
      ...
      try {
        ...
        Alert.alert("載入失敗", `無法從伺服器獲取測驗數據: ${errorMessage}`);
        setQuiz(null);
      } finally {
        setIsJumping(false);
  ...
  if (!quiz || quiz.questions.length === 0) {
    return (
      <View style={s.center}>
        ...
        <Text style={s.muted}>
          請確認該單元是否有資料 ({quizVolume}冊, {quizLesson}課)
        </Text>
        {!!apiError && (
          <Text style={[s.muted, { color: "red", marginTop: 8 }]}>
            API 錯誤: {apiError}
          </Text>
        )}
      </View>
          ...
          needsManualNext ? (
            ...
            <Pressable
              style={[
                s.btn,
                s.btnPrimary,
                (!hasAnswer || isJumping) && s.btnDisabled,
              ]}
    ...
    case "order":
      ...
      return (
        <View
          style={[
            s.orderContainer,
            isJumping && (isOrderCorrect ? s.cardCorrect : s.cardWrong),
          ]}
        >
          {isJumping && (
            <Text style={{ fontSize: 14, fontWeight: "bold", padding: 8 }}>
            ...
            return (
              <View key={id} style={s.orderItem}>
                ...
                <View style={s.orderBtns}>
                  <Pressable
                    style={[s.orderBtn, isJumping && s.btnDisabled]}
                    onPress={() => {
                      if (idx === 0) return;
                      const copy = [...list];
